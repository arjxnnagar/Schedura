import { Calendar, CalendarDaysIcon, icons, LayoutDashboardIcon, LogOutIcon, UsersIcon, Wand2Icon } from "lucide-react";
...
const Sidebar = ({ isOpen, setIsOpen }) => {
  ...
  const navItems = [
    ...
    { name: "Schedura", icon: CalendarDaysIcon, path: "/schedura" },
    ...
    >
        ...
        {navItems.map((item) => {
          ...
          return (
            <NavLink
              ...
              className={`flex gap-3 items-center px-3 py-2.5 rounded text-sm transition-all duration-150 border ${isActive ? "bg-red-50 text-red-500 border-red-100" : "text-slate-500 hover:bg-slate-50 border-transparent hover:text-slate-700"}`}
            >
              <item.icon
              ...
              {item.name}
              {isActive && (
                <span className="ml-auto w-1.25 h-5 rounded-full bg-red-500" />
              )}
            </NavLink>
          );
        })}
      ...
      <div className="p-4 border-t border-slate-100">
        <div className="flex items-center gap-3 p-2 rounded-xl hover:bg-slate-50 transition-colors">
          ...
          <div className="flex-1 min-w-0 text-sm text-slate-400 truncate">
            <div className="text-slate-800 ">{user.name}</div>
            <div>{user.email}</div>
          </div>
        </div>
        
        <button className="mt-1 flex items-center gap-2 p-3 w-full rounded text-sm text-slate-500 hover:bg-red-50 hover:text-red-500 transition-all duration-150"
        onClick={logout}>
